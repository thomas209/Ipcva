...
$carga_trimestre_id=(isset($_GET['carga_trimestre_id']) && $_GET['carga_trimestre_id'] != '' ) ? intval($_GET['carga_trimestre_id']) : '';

if($carga_trimestre_id!=''){
	$sql="select ct.*, u.nombre || ' '|| u.apellido as empleado from cargas_trimestres ct, usuarios u where ct.usuario_id=u.usuario_id and ct.carga_trimestre_id=$carga_trimestre_id";
	...
	$sql="select ctd.*, '('||cc.nombre||') '||cc.descripcion as centro_costos_nombre from cargas_trimestres_detalles ctd, cargas_trimestres ct, centros_costos cc where ctd.carga_trimestre_id=ct.carga_trimestre_id and ctd.centro_costos_id=cc.centro_costos_id and ctd.carga_trimestre_id=$carga_trimestre_id";
	if($perfil_id_logueado!=1)$sql.=" and u.usuario_id=$usuario_id_logueado";
	if($debug==1)echo $sql.'<br>';
	$rs=pg_query($sql);
...
$sql="SELECT usuario_id, nombre, apellido FROM usuarios WHERE usuario_id=$usuario_id order by apellido";
if($debug==1)echo $sql.'<br>';
$rs=pg_query($sql);
	...
	<div id="wrapper">
        ...
        <div id="page-wrapper" class="distribuidores">
            <div class="container-fluid">
                <div class="row">
                   <div class="col-md-9">
                        <h1 class="page-header">Cargas de trabajo</h1>
                    </div>
              ...
              <form name="altatrimestre" id="altatrimestre" method="POST" action="carga_trimestre_editar_pr.php" onsubmit="return checkForm();">
				<div class="row no-mar">
					<div class="col-md-12">
						<input type="hidden" name="contador" id="contador" value="<?=$contador?>">
						<input type="hidden" name="carga_trimestre_id" id="carga_trimestre_id" value="<?=$carga_trimestre_id?>">
						<h3 class="mt-15">Datos Generales</h3>
						<?
						if($perfil_id_logueado==5){
						?>
							<input type="hidden" name="usuario_id" id="usuario_id" value="<?=$usuario_id_logueado?>">
						<?
						}
						else{
						?>
				...
				<div class="row no-mar">
					<div class="col-md-12">						
						...
						<div class="row">
							...
							<div class="col-md-1">
								<button type="button" class="btn btn-primary" id="agregar"><i class="glyphicon glyphicon-plus" style="width: 10px;"></i> Agregar</button>
							</div>
						</div>
					</div>
				</div>
							...
							<table class="table">
								...
								<tbody id="tablebody">
									...
									?>
										<tr id="fila_<?=$i?>">
											<td>
												<input type="hidden" name="centro_costos_id_fila_<?=$i?>" id="centro_costos_id_fila_<?=$i?>" value="<?=$carga_trimestre_detalle['centro_costos_id']?>" readonly>
												<input type="text" class="form-control" name="descripcion_<?=$i?>" id="descripcion_<?=$i?>" readonly value="<?=$carga_trimestre_detalle['centro_costos_nombre']?>">
											</td>
											<td>
									...
									<?
										$i++;
										$porcentaje_total=$porcentaje_total+$carga_trimestre_detalle['porcentaje'];
									}
									?>
								</tbody>
								<tfoot>
									<td><b>Total</b></td>
									<td><input type="text" name="porcentaje_total" id="porcentaje_total" class="form-control" readonly style="text-align: right;" value="<?=$porcentaje_total?>"></td>
									<td></td>
