...
$sql="SELECT centro_costos_id, centro_costos_id_padre, cuenta_haberes_id, cuenta_carga_social_id, nombre, descripcion, habilitado from centros_costos where centro_costos_id=$centro_costos_id order by nombre";
$rs=pg_query($sql);
$centro_costos_id=pg_fetch_result($rs, 0, 'centro_costos_id');
$centro_costos_id_padre=pg_fetch_result($rs, 0, 'centro_costos_id_padre');
$cuenta_haberes_id=pg_fetch_result($rs, 0, 'cuenta_haberes_id');
$cuenta_carga_social_id=pg_fetch_result($rs, 0, 'cuenta_carga_social_id');
$nombre=pg_fetch_result($rs, 0, 'nombre');
$descripcion=pg_fetch_result($rs, 0, 'descripcion');
$habilitado=pg_fetch_result($rs, 0, 'habilitado');
pg_free_result($rs);
////////////////////
$sql="SELECT cuenta_haberes_id, descripcion FROM cuentas_haberes order by cuenta_haberes_id";
$rs=pg_query($sql);
$cuenta_haberes=pg_fetch_all($rs);
pg_free_result($rs);
////////////////////
$sql="SELECT cuenta_carga_social_id, descripcion FROM cuentas_cargas_sociales order by cuenta_carga_social_id";
$rs=pg_query($sql);
$cuenta_cargas_sociales=pg_fetch_all($rs);
pg_free_result($rs);
?>
<!DOCTYPE html>
<html lang="es">
	...
	<div id="wrapper">
        ...
        <div id="page-wrapper" class="distribuidores">
            <div class="container-fluid">
                <div class="row">
                   <div class="col-md-9">
                        <h1 class="page-header">Editar Centro de Costo</h1>
                    </div>
				...
				<form name="altacentrocostos" id="altacentrocostos" method="POST" action="centro_costos_editar_pr.php" onsubmit="checkForm()">
	               <input type="hidden" name="centro_costos_id" value="<?=$centro_costos_id?>">
	                <div class="row no-mar">
						<div class="col-md-12">
							<h3 class="mt-15">Detalle</h3>
							<div class="form-group col-md-2">
								<label>Nombre</label>
								<input type="text" class="form-control" name="nombre" id="nombre" value="<?=$nombre?>" style="text-align: left">
							</div>
							<div class="form-group col-md-10">
								<label>Descripcion</label>
								<input type="text" class="form-control" name="descripcion" id="descripcion" value="<?=$descripcion?>" style="text-align: left">
							</div>
							<div class="form-group col-md-2">
							 <label>Padre</label>
								  <select name="centro_costos_id_padre" class="form-control">
										...
										$rs=pg_query($sql);
										$centro_costo_padre=pg_fetch_all($rs);
										pg_free_result($rs);

										foreach ($centro_costo_padre as $padre) {
											if($padre['centro_costos_id']==$centro_costos_id_padre){
												$selected='selected';
											}
											else{
												$selected='';
											}
											echo "<option value=\"{$padre['centro_costos_id']}\" $selected>{$padre['nombre']}</option>";
										}
										?>
								  </select>
							</div>
							<div class="form-group col-md-3">
								<label>Haberes</label>
									<select class="form-control" name="cuenta_haberes_id">
										...
										<?
										foreach ($cuenta_haberes as $haberes) {
											if($haberes['cuenta_haberes_id']==$cuenta_haberes_id){
												$selected='selected';
											}
											else{
												$selected='';
											}
											echo "<option value=\"{$haberes['cuenta_haberes_id']}\" $selected>{$haberes['descripcion']}</option>";
										}
										?>
									</select>
							</div>
							<div class="form-group col-md-3">
								<label>Cargas Sociales</label>
									<select class="form-control" name="cuenta_carga_social_id">
										<option value="" selected>Seleccione</option>
										<?
										foreach ($cuenta_cargas_sociales as $cargas_sociales) {
											if($cargas_sociales['cuenta_carga_social_id']==$cuenta_carga_social_id){
												$selected='selected';
											}
											else{
												$selected='';
											}
											echo "<option value=\"{$cargas_sociales['cuenta_carga_social_id']}\" $selected>{$cargas_sociales['descripcion']}</option>";
										}
										?>
									</select>
							</div>
